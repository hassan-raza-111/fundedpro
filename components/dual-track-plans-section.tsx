...
//     const selectedPlan = plans[activeMobilePlanIndex];
//     return rows.map((row) => ({
//       label: row.label,
//       value: row.values[activeMobilePlanIndex],
//     }));
//   }, [plans, rows, activeMobilePlanIndex]);

//   // Callbacks
//   const handlePlanTypeChange = useCallback((planType: PlanType) => {
//     setActivePlanType(planType);
//     setActiveMobilePlanIndex(0); // Reset mobile selection
//   }, []);
...
//             className={`relative px-4 sm:px-6 py-3 font-semibold text-sm sm:text-base transition-all duration-300 ${
//               activePlanType === planType.id
//                 ? "bg-white text-[#1a237e] shadow-lg rounded-lg"
//                 : "text-gray-300 hover:text-white bg-transparent"
//             } ${index === 0 ? "rounded-l-lg" : ""} ${
//               index === PLAN_TYPES.length - 1 ? "rounded-r-lg" : ""
//             }`}
//           >
//             {planType.name}
//           </button>
...
//               className={`shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 text-xs font-semibold flex items-center justify-center transition-all duration-300 ${
//                 isActive
...
//                   : "text-white border-white/40 bg-white/5 hover:bg-white/10 hover:border-white/60 hover:scale-105"
//               }`}
...
//                 className="w-full bg-gradient-to-r from-[#00f7ff] to-[#0072ff] hover:from-[#00e0ff] hover:to-[#005ce6] text-white font-bold py-3 rounded-xl leading-tight flex items-center justify-center shadow-[0_3px_12px_rgba(0,247,255,0.3)] hover:shadow-[0_5px_18px_rgba(0,247,255,0.45)] focus-visible:ring-2 focus-visible:ring-[#00f7ff]/40 min-h-[55px] text-center cursor-pointer transition-all duration-300"
//               >
...
export function DualTrackPlansSection() {
  ...
  // Callbacks - Simplified for better performance
  const handlePlanTypeChange = useCallback((planType: PlanType) => {
  ...
  const handleMobilePlanSelect = useCallback((index: number) => {
    ...
    setClickFeedback(`Plan selected: ${plans[index]?.label || index}`);
    setActiveMobilePlanIndex(index);
    setTimeout(() => setClickFeedback(""), 2000);
  }, [plans]);
  ...
  const renderPlanTypeSelector = () => (
    <div className="flex justify-center mb-8 relative z-20">
      <div className="inline-flex bg-white/10 backdrop-blur-md rounded-xl p-1 border border-white/20 relative z-20">
        {PLAN_TYPES.map((planType, index) => (
          <button
            key={planType.id}
            onClick={() => handlePlanTypeChange(planType.id as PlanType)}
            onTouchStart={() => handlePlanTypeChange(planType.id as PlanType)}
            className={`relative px-4 sm:px-6 py-3 font-semibold text-sm sm:text-base transition-all duration-200 cursor-pointer touch-manipulation focus:outline-none focus:ring-2 focus:ring-white/50 ${
              activePlanType === planType.id
                ? "bg-white text-[#1a237e] shadow-lg rounded-lg"
                : "text-gray-300 hover:text-white bg-transparent"
            } ${index === 0 ? "rounded-l-lg" : ""} ${
  ...
  const renderMobileCards = () => (
    <div className="block xl:hidden mt-4 relative z-20">
      {/* Account size selector chips */}
      <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-4 pt-2 px-4 relative z-20" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch' }}>
        {plans.map((plan, idx) => {
          const isActive = idx === activeMobilePlanIndex;
          return (
            <button
              ...
              className={`shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-full border-2 text-xs font-semibold flex items-center justify-center transition-all duration-200 cursor-pointer touch-manipulation focus:outline-none focus:ring-2 focus:ring-[#00f7ff]/50 ${
                isActive
                  ? "bg-gradient-to-br from-[#00f7ff] to-[#0072ff] text-white border-[#00f7ff] shadow-[0_4px_15px_rgba(0,247,255,0.4)] transform scale-110"
      ...
      {/* Selected plan card */}
      <div className="mt-2 bg-gradient-to-br from-white/10 via-white/8 to-white/5 backdrop-blur-md rounded-2xl border border-white/20 overflow-hidden shadow-2xl relative z-20">
        <div className="p-4 sm:p-5">
          <div className="text-center">
            <div className="text-sm text-gray-300 mb-1">Account Size</div>
            <div className="text-2xl sm:text-3xl font-bold text-white mb-4">
            ...
            <div className="flex flex-col items-center gap-2">
              <button
                ...
                className="w-full bg-gradient-to-r from-[#00f7ff] to-[#0072ff] hover:from-[#00e0ff] hover:to-[#005ce6] text-white font-bold py-3 rounded-xl leading-tight flex items-center justify-center shadow-[0_3px_12px_rgba(0,247,255,0.3)] hover:shadow-[0_5px_18px_rgba(0,247,255,0.45)] focus-visible:ring-2 focus-visible:ring-[#00f7ff]/40 min-h-[55px] text-center cursor-pointer transition-all duration-200 touch-manipulation"
                style={{ 
                  WebkitTapHighlightColor: 'transparent', 
                  userSelect: 'none', 
                  zIndex: 20, 
  ...
  const renderDesktopTable = () => (
    <div className="block mt-8 relative z-20">
      <div className="w-full max-w-[1200px] xl:max-w-[1400px] 2xl:max-w-[1600px] mx-auto rounded-2xl overflow-hidden border border-white/20 bg-gradient-to-br from-white/8 via-white/6 to-white/4 backdrop-blur-md shadow-2xl">
        {/* Header Row */}
        <div className="grid grid-cols-7">
          <div className="p-4 xl:p-5 text-sm xl:text-base font-semibold text-gray-300 bg-[rgba(30,41,59,0.85)] backdrop-blur-md">
            ...
            >
              ...
              <div className="mt-3 xl:mt-4 flex flex-col items-center gap-2">
                <button
                  onClick={handleGetPlanClick}
                  onTouchStart={handleGetPlanClick}
                  className="w-4/5 xl:w-3/4 mx-auto bg-gradient-to-r from-[#00f7ff] to-[#0072ff] hover:from-[#00e0ff] hover:to-[#005ce6] text-white font-bold py-2 xl:py-2.5 rounded-lg leading-tight flex items-center justify-center shadow-[0_2px_8px_rgba(0,247,255,0.3)] hover:shadow-[0_3px_12px_rgba(0,247,255,0.45)] focus-visible:ring-2 focus-visible:ring-[#00f7ff]/40 min-h-[40px] xl:min-h-[45px] text-center cursor-pointer transition-all duration-200 touch-manipulation"
                  style={{ 
                    WebkitTapHighlightColor: 'transparent', 
                    userSelect: 'none',
                    zIndex: 20,
