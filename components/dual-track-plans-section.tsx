...
//       <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-4 pt-2 px-4 scrollbar-hide">
//         {plans.map((plan, idx) => {
...
//               className={`shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 text-xs font-semibold flex items-center justify-center transition-all duration-200 ${
//                 isActive
//                   ? "bg-gradient-to-br from-[#00f7ff] to-[#0072ff] text-white border-[#00f7ff] shadow-[0_4px_15px_rgba(0,247,255,0.4)] transform scale-110"
...
const STELLAR_2STEP_ROWS = [
  ...
  { label: 'News Trading', values: ['✔', '✔', '✔', '✔', '✔', '✔'] },
  {
    label: 'Performance Reward Upto',
    values: ['95%', '95%', '95%', '95%', '95%', '95%'],
  },
  {
    label: 'Minimum Trading Days',
    values: ['3 Days', '3 Days', '3 Days', '3 Days', '3 Days', '3 Days'],
  },
  {
  ...
  },
  {
    ...
    values: ['$59', '$119', '$199', '$299', '$549', '$1099'],
  },
] as const;
...
const STELLAR_1STEP_ROWS = [
  ...
  {
    label: 'Minimum Trading Days',
    values: ['2 Days', '2 Days', '2 Days', '2 Days', '2 Days', '2 Days'],
  },
  {
    label: 'First withdrawal',
    values: ['3 Days', '3 Days', '3 Days', '3 Days', '3 Days', '3 Days'],
  ...
  {
    label: 'Refundable Fee',
    values: ['$65', '$129', '$219', '$329', '$569', '$1099'],
  },
] as const;
...
export function DualTrackPlansSection() {
  const [activePlanType, setActivePlanType] =
    useState<PlanType>('stellar-2step');
  const [activeMobilePlanIndex, setActiveMobilePlanIndex] = useState(0);
  const [clickFeedback, setClickFeedback] = useState<string>('');
  ...
  const handleMobilePlanSelect = useCallback(
    (index: number) => {
      console.log('Mobile plan selected:', index);
      setClickFeedback(`Plan selected: ${plans[index]?.label}`);
      setActiveMobilePlanIndex(index);
  ...
  const renderPlanTypeSelector = () => (
    <div className="relative z-10 mb-8 flex justify-center">
      <div
        className="inline-flex rounded-xl border border-white/20 bg-white/10 p-1 backdrop-blur-md"
      ...
      >
        {PLAN_TYPES.map((planType, index) => (
          <button
            ...
            onClick={() => handlePlanTypeChange(planType.id as PlanType)}
            onTouchStart={() => handlePlanTypeChange(planType.id as PlanType)}
            className={`relative cursor-pointer px-4 py-3 text-sm font-semibold transition-all duration-200 focus:ring-2 focus:ring-white/50 focus:outline-none sm:px-6 sm:text-base ${
              activePlanType === planType.id
                ? 'rounded-lg bg-white text-[#1a237e] shadow-lg'
  ...
  const renderMobileCards = () => (
    <div className="mt-4 block xl:hidden">
        ...
        {plans.map((plan, idx) => {
          ...
          return (
            <button
              ...
              onClick={() => handleMobilePlanSelect(idx)}
              onTouchStart={() => handleMobilePlanSelect(idx)}
              className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-full border-2 text-xs font-semibold transition-all duration-200 ${
                isActive
                  ? 'scale-110 border-[#00f7ff] bg-gradient-to-br from-[#00f7ff] to-[#0072ff] text-white shadow-[0_4px_15px_rgba(0,247,255,0.4)]'
                  : 'border-white/40 bg-white/5 text-white hover:scale-105 hover:border-white/60 hover:bg-white/10 active:scale-95'
              }`}
              style={{
                touchAction: 'manipulation',
                userSelect: 'none',
      ...
      {/* Selected plan card */}
      <div className="mt-2 overflow-hidden rounded-2xl border border-white/20 bg-gradient-to-br from-white/10 via-white/8 to-white/5 shadow-2xl backdrop-blur-md">
        <div className="p-4 sm:p-5">
          <div className="text-center">
            <div className="mb-1 text-sm text-gray-300">Account Size</div>
            ...
            <div className="flex flex-col items-center gap-2">
              <Button
                onClick={handleGetPlanClick}
                onTouchStart={handleGetPlanClick}
                className="flex min-h-[55px] w-full cursor-pointer items-center justify-center rounded-xl bg-gradient-to-r from-[#00f7ff] to-[#0072ff] py-3 text-center leading-tight font-bold text-white shadow-[0_3px_12px_rgba(0,247,255,0.3)] transition-all duration-200 hover:from-[#00e0ff] hover:to-[#005ce6] hover:shadow-[0_5px_18px_rgba(0,247,255,0.45)] focus-visible:ring-2 focus-visible:ring-[#00f7ff]/40"
              >
                <div className="flex flex-col items-center justify-center text-center">
            ...
            {selectedPlanDetails.map(item => (
              ...
              >
                <div className="flex items-center gap-2 font-medium text-gray-200">
                  <Info className="text-brand-cyan h-4 w-4" />
                  <span className="text-sm">{item.label}</span>
                </div>
  ...
  const renderDesktopTable = () => (
    <div className="mt-8 hidden xl:block">
      <div className="mx-auto w-full max-w-[1200px] overflow-hidden rounded-2xl border border-white/20 bg-gradient-to-br from-white/8 via-white/6 to-white/4 shadow-2xl backdrop-blur-md xl:max-w-[1400px] 2xl:max-w-[1600px]">
        {/* Header Row */}
        <div className="grid grid-cols-7">
          <div className="bg-[rgba(30,41,59,0.85)] p-4 text-sm font-semibold text-gray-300 backdrop-blur-md xl:p-5 xl:text-base">
            Account Size
          </div>
          {plans.map(plan => (
            <div
              key={plan.label}
              className="from-brand-cyan/30 via-brand-cyan/20 bg-gradient-to-br to-[#1a237e]/70 p-4 text-center text-white xl:p-5"
            >
              <div className="text-sm font-bold whitespace-nowrap xl:text-base">
                {plan.label}
              </div>
              <div className="mt-3 flex flex-col items-center gap-2 xl:mt-4">
                <Button
                  onClick={handleGetPlanClick}
                  onTouchStart={handleGetPlanClick}
                  className="mx-auto flex min-h-[40px] w-4/5 cursor-pointer items-center justify-center rounded-lg bg-gradient-to-r from-[#00f7ff] to-[#0072ff] py-2 text-center leading-tight font-bold text-white shadow-[0_2px_8px_rgba(0,247,255,0.3)] transition-all duration-200 hover:from-[#00e0ff] hover:to-[#005ce6] hover:shadow-[0_3px_12px_rgba(0,247,255,0.45)] focus-visible:ring-2 focus-visible:ring-[#00f7ff]/40 xl:min-h-[45px] xl:w-3/4 xl:py-2.5"
                >
                  <div className="flex flex-col items-center justify-center text-center">
                    <span className="text-xs font-semibold tracking-wide xl:text-sm">
                      Get Plan
                    </span>
        ...
        {rows.map((row, idx) => (
          <div
            ...
            className={`grid grid-cols-7 ${idx === 0 ? '' : 'border-t border-white/10'}`}
          >
            <div className="bg-[rgba(30,41,59,0.85)] p-3 text-left text-sm font-medium text-gray-200 backdrop-blur-md xl:p-4 xl:text-base">
              {row.label}
              {row.label === 'Refundable Fee' && (
            ...
            {row.values.map((value, i) => (
              <div
                key={i}
                className="bg-transparent p-3 text-center text-sm whitespace-nowrap text-white xl:p-4 xl:text-base"
              >
                {value}
              </div>
            ))}
  ...
  const renderPlanContent = () => {
    const planTitles = {
      'stellar-2step': {
        title: 'Stellar 2-Step Challenge',
        subtitle: 'Two-phase evaluation process',
      },
      'stellar-1step': {
        title: 'Stellar 1-Step Challenge',
        subtitle: 'Single-phase evaluation process',
      },
      'stellar-lite': {
        title: 'Stellar Lite Challenge',
        subtitle: 'Simplified evaluation process',
      },
    };
  ...
  return (
    <section
      id="plans"
      className="bg-brand-gradient relative scroll-mt-24 overflow-hidden py-20 md:scroll-mt-28"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-16 text-center">
          ...
          <p className="mx-auto max-w-sm px-2 text-sm leading-relaxed text-gray-300 sm:max-w-lg sm:px-4 sm:text-base md:max-w-4xl md:px-4 md:text-base lg:max-w-4xl lg:text-xl xl:max-w-5xl xl:text-2xl">
            Multiple flexible funding models designed for different trader
        ...
        {renderPlanTypeSelector()}
        {clickFeedback && (
          <div className="pointer-events-none fixed top-4 left-1/2 z-50 -translate-x-1/2 transform animate-pulse rounded-lg bg-green-500 px-4 py-2 text-white shadow-lg">
            {clickFeedback}
          </div>
        )}
        {renderPlanContent()}
