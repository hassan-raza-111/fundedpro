...
export function FAQSection() {
  ...
  const accordionItems = useMemo(() => 
    FAQ_DATA.map((faq) => (
      <AccordionItem
        key={faq.question}
        value={faq.question}
        className="border border-white/10 rounded-lg bg-white/5 hover:bg-white/10 transition-colors duration-200 motion-safe:transition-all focus-within:ring-1 focus-within:ring-brand-cyan/40"
      >
        <AccordionTrigger
          aria-label={`Question: ${faq.question}`}
          className="px-4 sm:px-6 py-4 text-left text-white hover:text-brand-cyan transition-colors duration-200 hover:no-underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-cyan/40 rounded-lg"
        >
          <span className="font-semibold text-base sm:text-lg">
            {faq.question}
          </span>
        </AccordionTrigger>
        <AccordionContent className="px-4 sm:px-6 pb-4 text-gray-300 leading-relaxed text-sm sm:text-base">
          {faq.answer}
        </AccordionContent>
      </AccordionItem>
    )), []
  ...
  return (
    <section
      ...
      className="scroll-mt-24 md:scroll-mt-28 scroll-smooth py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-[#1a237e] to-[#0f1419]"
    >
