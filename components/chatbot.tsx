...
const FAQ_DATA = [
  ...
  },
  {
    ...
    answer:
      "Our challenge accounts offer a 100-day evaluation period - that's 3x longer than most competitors who only provide 30-60 days. This extended timeframe gives you more confidence to develop and refine your trading strategy.",
  },
  {
...
const findBestFAQMatch = (userQuestion: string): string | null => {
  ...
  // Direct match first
  const directMatch = FAQ_DATA.find(
    faq =>
...
const ChatMessage = memo(({ message }: { message: Message }) => {
  ...
  return (
    <div className={`flex ${isBot ? 'justify-start' : 'justify-end'} mb-4`}>
      <div
        className={`flex max-w-[88%] items-start gap-3 ${isBot ? 'flex-row' : 'flex-row-reverse'}`}
      >
        <div
        ...
        <div className={`flex flex-col ${isBot ? 'items-start' : 'items-end'}`}>
          <div
            className={`w-full min-w-0 rounded-2xl px-4 py-3 ${
              isBot
                ? 'border border-white/20 bg-white/10 text-white backdrop-blur-sm'
                : 'bg-brand-cyan text-black'
            }`}
          >
            <p className="text-left text-sm leading-6 tracking-wide break-words hyphens-none whitespace-normal">
              {message.text}
            </p>
          </div>
          <span className="mt-1 text-xs text-white/50">{timeString}</span>
        </div>
      </div>
    </div>
  );
...
const TypingIndicator = memo(() => (
  <div className="mb-4 flex justify-start">
    <div className="flex items-start gap-3">
      <div className="bg-brand-cyan flex h-8 w-8 items-center justify-center rounded-full text-black">
...
const QuickReplies = memo(
  ({ onSendMessage }: { onSendMessage: (message: string) => void }) => (
    <div className="mt-4">
      <p className="mb-3 text-xs text-white/60">Quick questions:</p>
      <div className="flex flex-col gap-2">
        {QUICK_REPLIES.map((reply, index) => (
          <button
            key={index}
            onClick={() => onSendMessage(reply)}
            className="rounded-lg border border-white/20 bg-white/10 px-4 py-2.5 text-left text-sm text-white transition-all duration-200 hover:scale-[1.02] hover:bg-white/20"
          >
            {reply}
          </button>
        ))}
...
const FloatingChatButton = memo(
  ({ onClick, isOpen }: { onClick: () => void; isOpen: boolean }) => (
    <button
      onClick={onClick}
      className={`bg-brand-cyan hover:bg-brand-cyan/90 animate-glow fixed right-6 bottom-6 z-50 h-14 w-14 rounded-full text-black shadow-lg transition-all duration-300 hover:scale-110 hover:shadow-xl ${
  ...
  ({
    ...
    <div className="flex flex-shrink-0 items-center justify-between border-b border-white/10 p-4">
      <div className="flex items-center gap-3">
        <div className="bg-brand-cyan flex h-8 w-8 items-center justify-center rounded-full">
    ...
    <div className="flex-shrink-0 border-t border-white/10 p-3">
      <div className="flex items-center gap-2">
        <Input
          ...
          className="focus:border-brand-cyan focus:ring-brand-cyan/20 h-10 flex-1 border-white/20 bg-white/10 text-sm text-white placeholder:text-white/50"
          disabled={isTyping}
        />
        <Button
          onClick={onSend}
          disabled={!inputValue.trim() || isTyping}
          className="bg-brand-cyan hover:bg-brand-cyan/90 h-10 w-10 flex-shrink-0 px-3 text-black"
          size="sm"
...
export const Chatbot = memo(({ className = '' }: ChatbotProps) => {
  ...
  const handleSendMessage = useCallback(
    async (customMessage?: string) => {
      ...
      const botResponse: Message = {
        ...
        text:
          faqAnswer ||
          'Sir, please contact our support team at support@thefundedpro.com for further assistance.',
        sender: 'bot',
        timestamp: new Date(),
      };
  ...
  // Only render the chat window when it's open
  if (!isOpen) {
    return (
  ...
  return (
    <div className={`fixed right-6 bottom-6 z-50 ${className}`}>
      ...
      <div className="translate-y-0 scale-100 opacity-100 transition-all duration-300 ease-in-out">
        <div
          className={`bg-brand-gradient flex flex-col rounded-2xl border border-white/20 shadow-2xl backdrop-blur-xl ${windowClasses} transition-all duration-300`}
        >
          <ChatHeader
